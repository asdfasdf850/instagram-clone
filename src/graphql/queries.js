...
export const SUGGEST_USERS = gql`
  query suggestUsers($limit: Int!, $followersIds: [uuid!]!, $createdAt: timestamptz!) {
    users(limit: $limit, where: { _or: [{ id: { _in: $followersIds } }, { created_at: { _gt: $createdAt } }] }) {
      id
      username
      name
      profile_image
...
// newest to oldest where the posts are not from users we are following
