...
export default function NotificationList({ setShowList, notifications, currentUserId }) {
  const classes = useNotificationListStyles()
  const listContainerRef = useRef()
  useOutsideClick(listContainerRef, () => setShowList(false))
  const [checkNotifications] = useMutation(CHECK_NOTIFICATIONS)

  useEffect(() => {
    checkNotifications({ variables: { userId: currentUserId, lastChecked: new Date().toISOString() } })
